#!/usr/bin/env python3
"""
Standalone schematic library builder - stdlib only, plus mcschematic.

Downloads the assskelad/minecraftbuildings CSV directly from HuggingFace
(no `datasets` package needed), converts each building to a .schem file with
a kid-readable name, and writes schematics_index.json for bot search.

    pip install mcschematic --break-system-packages
...
CSV_URL = "https://huggingface.co/datasets/assskelad/minecraftbuildings/resolve/main/dataset.csv"
CSV_PATH = os.environ.get("CSV_PATH", "/tmp/minecraftbuildings.csv")
...
def make_name(desc, idx):
    ...
    priority = [w for w in meaningful if w in PRIORITY_WORDS]

    seen, ordered = set(), []
    for w in priority + meaningful:
        if w not in seen:
            seen.add(w)
            ordered.append(w)
        if len(ordered) >= 4:
...
def download_csv():
    if os.path.exists(CSV_PATH) and os.path.getsize(CSV_PATH) > 1_000_000:
        print(f"Using cached CSV at {CSV_PATH}", flush=True)
        return
    print(f"Downloading dataset CSV to {CSV_PATH}...", flush=True)
    req = urllib.request.Request(CSV_URL, headers={"User-Agent": "Mozilla/5.0"})
    with urllib.request.urlopen(req, timeout=120) as resp, open(CSV_PATH, "wb") as out:
        while True:
            chunk = resp.read(1 << 20)
            if not chunk:
                break
            out.write(chunk)
    print(f"Downloaded {os.path.getsize(CSV_PATH)} bytes", flush=True)
...
def main():
    ...
    with open(CSV_PATH, newline="", encoding="utf-8", errors="replace") as f:
        ...
        for idx, row in enumerate(reader):
            desc = pick(row, "Description", "description") or ""
            raw = pick(row, "Block Info", "block_info", "BlockInfo")
            if not raw:
                skipped += 1
                continue
            try:
            ...
            for entry in blocks:
                ...
                if block in SKIP_BLOCKS:
                    continue
                schem.setBlock((int(x), int(y), int(z)), block)
                placed += 1
                if y > max_y:
                    max_y = y
            ...
            if ok % 200 == 0:
                print(f"  {ok} written, {skipped} skipped", flush=True)

    with open(INDEX_PATH, "w") as f:
        json.dump(index, f, indent=1)

    print(f"DONE: {ok} schematics in {OUT_DIR}, {skipped} skipped", flush=True)
    if unmapped_ids:
        print("Unmapped IDs:", dict(unmapped_ids.most_common(10)), flush=True)
    for s in list(index.keys())[:12]:
